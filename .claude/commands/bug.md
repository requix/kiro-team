...
- Create the plan in the `specs/` directory with filename: `issue-{issue_number}-adw-{adw_id}-sdlc_planner-{descriptive-name}.md`
  - Replace `{descriptive-name}` with a short, descriptive name based on the bug (e.g., "fix-agent-config", "resolve-delegation-failure", "patch-prompt-reference")
...
- IMPORTANT: If the bug affects agent behavior or workflows:
...
<If the bug affects agent behavior, include a task to create an E2E test file. Your task should look like: "Read `.claude/commands/e2e/test_builder_agent.md` and `.claude/commands/e2e/test_agent_error_handling.md` and create a new E2E test file in `.claude/commands/e2e/test_<descriptive_name>.md` that validates the bug is fixed, be specific with the steps to prove the bug is fixed.">
